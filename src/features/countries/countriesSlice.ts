...
export const fetchNeighbor = createAsyncThunk<string,
    string,
    { rejectValue: Error, state: RootState }>(
    'countries/fetchNeighbor',
    async (borderCode) => {
        const url: string = `alpha/${borderCode}?fields=name`;
        return await countriesAPI.get(url)
            .then((res: AxiosResponse) => res.data as INeighbor)
            .then(array => array.name)
            .catch((e: Error) => {
...
const countriesSlice = createSlice({
    ...
    reducers: {
        ...
        setNeighborStatus: (state: ICountriesState, {payload}) => {
            state.neighborsStatus = payload;
        },
    },
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchNeighbor.fulfilled, (state, {payload}) => {
                if (state.detailed) {
                    state.neighbors.push(payload);
                } else {
                    state.neighbors = [];
                }
                state.neighborsStatus = 'idle';
            })
            .addCase(fetchNeighbor.rejected, (state, action) => {
                state.error = action.payload || action.error;
